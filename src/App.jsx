...
function WelcomeView() {
  return (
    <div style={{
      ...
      background: '#f2f5f9', color: '#a0b3c8', gap: 20,
      borderLeft: '1px solid #e0e8f2',
    }}>
      <svg viewBox="0 0 24 24" fill="#4a82c4" width="64" height="64" style={{opacity:0.18}}>
      ...
      <div style={{textAlign:'center'}}>
        <p style={{fontSize:20,fontWeight:600,color:'#52607a',marginBottom:6,letterSpacing:'-0.2px'}}>WA Dashboard</p>
        <p style={{fontSize:13,lineHeight:1.7,color:'#a0b3c8',maxWidth:260}}>Pilih percakapan dari daftar untuk mulai membalas pesan</p>
      </div>
    </div>
  )
}
...
function ProtectedShell() {
  ...
  return (
    <SocketProvider>
      <div className={`wa-app${chatId ? ' chat-open' : ''}`}>
        <IconBar />
        {isFullPage ? (
          ...
          <>
            <div className="wa-left-panel"><InboxPage /></div>
            <div className="wa-right-panel">
              {chatId ? <ChatPage key={chatId} chatId={chatId} /> : <WelcomeView />}
            </div>
          </>
        )}
      </div>
